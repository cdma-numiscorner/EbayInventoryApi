...
/**
     ...
     * @param  string $contentType The value for the Content-Type header. Check self::contentTypes['bulkMigrateListing'] to see the possible values for this operation
     *
     ...
     */
    public function bulkMigrateListing($bulk_migrate_listing, string $contentType = self::contentTypes['bulkMigrateListing'][0])
    {
        list($response) = $this->bulkMigrateListingWithHttpInfo($bulk_migrate_listing, $contentType);
        return $response;
    }
     ...
     */
    public function bulkMigrateListingWithHttpInfo($bulk_migrate_listing, string $contentType = self::contentTypes['bulkMigrateListing'][0])
    {
        $request = $this->bulkMigrateListingRequest($bulk_migrate_listing, $contentType);
            ...
            switch($statusCode) {
                case 200:
                    if ('\OpenAPI\EbayInventoryClient\Model\BulkMigrateListingResponse' === '\SplFileObject') {
                        ...
                        if ('\OpenAPI\EbayInventoryClient\Model\BulkMigrateListingResponse' !== 'string') {
     ...
     */
    public function bulkMigrateListingAsync($bulk_migrate_listing, string $contentType = self::contentTypes['bulkMigrateListing'][0])
    {
        return $this->bulkMigrateListingAsyncWithHttpInfo($bulk_migrate_listing, $contentType)
            ->then(
                function ($response) {
                    return $response[0];
                }
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function bulkMigrateListingAsyncWithHttpInfo($bulk_migrate_listing, string $contentType = self::contentTypes['bulkMigrateListing'][0])
    {
        $returnType = '\OpenAPI\EbayInventoryClient\Model\BulkMigrateListingResponse';
        $request = $this->bulkMigrateListingRequest($bulk_migrate_listing, $contentType);
     ...
     */
    public function bulkMigrateListingRequest($bulk_migrate_listing, string $contentType = self::contentTypes['bulkMigrateListing'][0])
    {
        ...
        if (isset($bulk_migrate_listing)) {
            if (stripos($headers['Content-Type'], 'application/json') !== false) {
                ...
                $httpBody = \GuzzleHttp\Utils::jsonEncode(ObjectSerializer::sanitizeForSerialization($bulk_migrate_listing));
            } else {
